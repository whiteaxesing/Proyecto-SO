...
const SendMessage = ({ scroll }) => {
  ...
  const sendMessage = async (e) => {
    ...
    if (input[0]==="/" && input[1]==="w"){
      ...
      await addDoc(collection(db, 'messages'), {
        text: CryptoJS.AES.encrypt("La temperatura en "+consulta.name+" es: "+Math.round(consulta.main.temp)+" F, "+"El clima está "+consulta.weather[0].main, "contraseña secreta").toString(),
        url: "",
        type: "",
        name: CryptoJS.AES.encrypt("BotClimatico", "contraseña secreta").toString(),
        uid: CryptoJS.AES.encrypt("Bot1", "contraseña secreta").toString(),
